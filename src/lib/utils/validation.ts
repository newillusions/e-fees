 ...
 */
function validateField<T>(value: unknown, rule: ValidationRule<T>, formData: T): string | null {
  ...
  if (rule.minLength && typeof value === 'string' && value.length < rule.minLength) {
    return (
      rule.errorMessage || `${String(rule.field)} must be at least ${rule.minLength} characters`
    );
  }
  ...
  if (rule.maxLength && typeof value === 'string' && value.length > rule.maxLength) {
    return (
      rule.errorMessage || `${String(rule.field)} must be no more than ${rule.maxLength} characters`
    );
  }
...
export const CommonValidationRules = {
  ...
  fee: {
    ...
    issueDate: {
      field: 'issue_date' as const,
      required: true,
      pattern: ValidationPatterns.dateYYMMDD
    },
