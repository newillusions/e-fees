<script lang="ts">
  ...
  ): { prefix: string; statusFolder: string; suffix: string } | null {
  ...
  <div style="padding: 16px;">
    ...
    {#if scanResult}
      ...
      >
        ...
        <div class="bg-emittiv-dark" style="border-radius: 4px; padding: 12px; text-align: center;">
          <div
            style="font-size: 20px; font-weight: 600; color: {scanResult.inconsistencies.length ===
            0
              ? '#4ade80'
              : '#facc15'};"
          >
      ...
      {#if scanResult.inconsistencies.length > 0}
        <div
          ...
          style="border: 1px solid; border-radius: 4px; overflow: hidden;"
        >
          <div
                  ...
                  <div style="flex: 1; min-width: 0;">
                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                      <span
                        ...
                        >{inconsistency.projectNumber}</span
                      >
                      <span
                        style="padding: 2px 8px; font-size: 10px; border-radius: 4px; {getTypeBadgeStyle(
                          inconsistency.type
                        )}"
                      >
                      ...
                      <div style="margin-top: 8px; font-size: 10px;">
                        ...
                        >
                          ...
                          <span style="font-family: monospace;">
                            {#if actualParts}
                              <span style="opacity: 0.5;">{actualParts.prefix}</span><span
                                style="background-color: rgba(248, 113, 113, 0.3); color: #f87171; padding: 0 2px; border-radius: 2px;"
                                >{actualParts.statusFolder}</span
                              ><span>{actualParts.suffix}</span>
                            {:else}
                              (unknown)
                            {/if}
                          </span>
                        ...
                        >
                          ...
                          <span style="font-family: monospace;">
                            {#if expectedParts}
                              <span style="opacity: 0.5;">{expectedParts.prefix}</span><span
                                style="background-color: rgba(74, 222, 128, 0.3); color: #4ade80; padding: 0 2px; border-radius: 2px;"
                                >{expectedParts.statusFolder}</span
                              ><span>{expectedParts.suffix}</span>
                            {:else}
                              (unknown)
                            {/if}
                          </span>
      ...
      {#if scanResult.errors.length > 0}
        <div
          style="margin-top: 16px; padding: 12px; background-color: rgba(113, 63, 18, 0.3); border: 1px solid #ca8a04; border-radius: 4px;"
        >
          <div style="font-size: 12px; font-weight: 500; color: #fcd34d; margin-bottom: 8px;">
            Scan Warnings
          </div>
          <ul
            style="font-size: 10px; color: #fef08a; list-style-type: disc; padding-left: 16px; margin: 0;"
          >
    ...
    {:else if !isScanning && !scanError}
      <div class="text-emittiv-light" style="text-align: center; padding: 32px 0;">
        <svg
          style="width: 40px; height: 40px; margin: 0 auto 8px; opacity: 0.5;"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
