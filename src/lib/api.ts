...
export class ApiClient {
   ...
   * 
   * @param rfp - Fee data without ID (will be generated by database) // formerly RFP
   * @returns Promise<Rfp | null> - Created Fee or null on failure // formerly RFP
   * 
   * @example
   * ```typescript
   * const newRfp = {
   ...
   * @param rfp - Complete Fee data with updated fields // formerly RFP
   * @returns Promise<Rfp | null> - Updated Fee or null on failure // formerly RFP
   * 
   * @example
   * ```typescript
   * const updatedRfp = {
   ...
   *   stage: "Under Review"
   * };
   * const result = await ApiClient.updateRfp("fee_id", updatedRfp); // formerly "rfp_id"
   * if (result) {
   *   console.log('Fee updated:', result.name); // formerly 'RFP updated'
   * }
   * ```
   * 
   ...
   * @example
   * ```typescript
   * const deleted = await ApiClient.deleteRfp("fee_id"); // formerly "rfp_id"
   * if (deleted) {
   *   console.log('Deleted Fee:', deleted.name); // formerly 'Deleted RFP'
   * }
   * ```
   * 
  ...
  static async writeFeeToJson(feeId: string): Promise<string | null> {
    try {
      const result = await invoke<string>('write_fee_to_json', { rfpId: feeId }); // rfpId param kept for backend compatibility
      return result;
    } catch (error) {
