...
impl Config {
    ...
    pub fn load() -> Self {
        ...
        defaults.insert("server.port".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(3201u64)));
        ...
        defaults.insert("ollama.url".to_string(), serde_yaml::Value::String("http://10.0.21.50:11434".to_string()));
        defaults.insert("ollama.model".to_string(), serde_yaml::Value::String("qwen3.5:9b".to_string()));
        defaults.insert("docling.url".to_string(), serde_yaml::Value::String("http://10.0.21.42:5001".to_string()));
        defaults.insert("stirling.url".to_string(), serde_yaml::Value::String("http://10.0.21.41:8080".to_string()));
        ...
        let ollama_url = env::var("OLLAMA_URL")
            .unwrap_or_else(|_| cm.get_str("ollama.url").unwrap_or_else(|| "http://10.0.21.50:11434".to_string()));

        let ollama_model = env::var("OLLAMA_MODEL")
            .unwrap_or_else(|_| cm.get_str("ollama.model").unwrap_or_else(|| "qwen3.5:9b".to_string()));

        let docling_url = env::var("DOCLING_URL")
            .unwrap_or_else(|_| cm.get_str("docling.url").unwrap_or_else(|| "http://10.0.21.42:5001".to_string()));

        let stirling_url = env::var("STIRLING_URL")
            .unwrap_or_else(|_| cm.get_str("stirling.url").unwrap_or_else(|| "http://10.0.21.41:8080".to_string()));
