...
fn main() {
    ...
    match serde_json::from_str::<FeeProposal>(test_data) {
        Ok(proposal) => {
        ...
        Err(e) => {
            ...
            // Try to parse as generic JSON to see what's different
            match serde_json::from_str::<serde_json::Value>(test_data) {
                Ok(json) => {
                    println!("Raw JSON parses fine:");
                    println!("{:#}", json);
                }
                Err(json_e) => {
                    println!("Even JSON parsing failed: {}", json_e);
                }
            }
        }
    }
