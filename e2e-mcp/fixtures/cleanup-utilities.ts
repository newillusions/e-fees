...
export class SafeTestDataCleaner {
  ...
  static async cleanupAllTestData(): Promise<CleanupReport> {
    ...
    try {
      ...
      report.totalDeleted = Object.values(report.deletedCounts).reduce(
        (sum, count) => sum + count,
        0
      );
    } catch (error) {
...
export const cleanupByTimestamp = (timestamp: string) =>
  SafeTestDataCleaner.cleanupByTimestamp(timestamp);
