...
export class SafeTestDataGenerator {
  ...
  generateSafeProjects(count: number = 1): SafeProjectData[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      projects.push({
        ...
        description: this.generateSafeName(
          `E2E testing ${projectType.toLowerCase()} in ${location.location}. This is test data for automated testing.`,
          'Description'
        ),
  ...
  generateSafeCompanies(count: number = 1): SafeCompanyData[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      companies.push({
        ...
        description: this.generateSafeName(
          `E2E testing company specializing in ${industry.toLowerCase()}. Test data for automated testing.`,
          'Description'
        ),
  ...
  generateSafeContacts(count: number = 1, companyIds?: string[]): SafeContactData[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      contacts.push({
        ...
        notes: this.generateSafeName(
          `E2E testing contact for automated tests. Contact #${i + 1}`,
          'Notes'
        )
      });
    }
  ...
  generateSafeProposals(
    ...
    const statuses: SafeProposalData['status'][] = [
      'draft',
      'sent',
      'approved',
      'rejected',
    ...
    for (let i = 0; i < count; i++) {
      ...
      proposals.push({
        ...
        description: this.generateSafeName(
          `E2E testing proposal for ${proposalType.toLowerCase()}. Automated test data.`,
          'Description'
        ),
  ...
  generateCompleteTestDataset(
    ...
    const { projectCount = 3, companyCount = 2, contactCount = 4, proposalCount = 3 } = options;
    ...
    const edgeProjects: SafeProjectData[] = [
      {
        ...
        description: this.generateSafeName(
          'Testing special characters: àáâãäåæçèéêëìíîïñòóôõöøùúûüýÿ 中文 العربية',
          'Description'
        ),
        location: 'Dubai',
        country: 'UAE',
        area: 'Downtown Dubai',
        city: 'Dubai',
        client: this.generateSafeName(
          'Client with Very Long Name That Exceeds Normal Character Limits for Testing Purposes',
          'Client'
        ),
        status: 'active'
      }
    ...
    const edgeCompanies: SafeCompanyData[] = [
      {
        ...
        description: this.generateSafeName(
          'Testing unicode: 🏢🏗️🏠 àáâã العربية 中文 русский',
          'Description'
        ),
    ...
    const edgeProposals: SafeProposalData[] = [
      {
        name: this.generateSafeName(
          'Proposal with "Special Chars" & Very Long Title That Tests Field Limits',
          'Proposal'
        ),
        description: this.generateSafeName(
          'Testing maximum description length with special characters: àáâãäåæç 中文 العربية',
          'Description'
        ),
        status: 'draft',
        amount: 999999.99,
 ...
 */
export const generateSafeProjectData = (count: number = 1) =>
  safeTestDataGenerator.generateSafeProjects(count);
export const generateSafeCompanyData = (count: number = 1) =>
  safeTestDataGenerator.generateSafeCompanies(count);
export const generateSafeContactData = (count: number = 1) =>
  safeTestDataGenerator.generateSafeContacts(count);
export const generateSafeProposalData = (count: number = 1) =>
  safeTestDataGenerator.generateSafeProposals(count);
export const generateCompleteTestDataset = (
  options?: Parameters<SafeTestDataGenerator['generateCompleteTestDataset']>[0]
