...
export class MCPClient {
  ...
  async executeJS(code: string): Promise<any> {
    try {
      ...
      throw new Error(
        `JS execution failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async keyboardShortcut(shortcut: KeyboardShortcut | string): Promise<void> {
    try {
      ...
      throw new Error(
        `Keyboard shortcut failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  async navigateTo(
    page: 'dashboard' | 'projects' | 'companies' | 'contacts' | 'proposals'
  ): Promise<void> {
    const shortcuts = {
      dashboard: 'Cmd+1',
      projects: 'Cmd+2',
  ...
  ): Promise<TestResult<{ projectNumber: string }>> {
  ...
  private async getLastCreatedProjectNumber(): Promise<string> {
    ...
    const result = await mcp__surrealdb_emittiv__query({
      query_string:
        'SELECT project_number FROM projects WHERE name CONTAINS "DELETE ME" ORDER BY created_at DESC LIMIT 1'
    });
  ...
  private async getLastCreatedCompanyId(): Promise<string> {
    ...
    const result = await mcp__surrealdb_emittiv__query({
      query_string:
        'SELECT id FROM company WHERE name CONTAINS "DELETE ME" ORDER BY created_at DESC LIMIT 1'
    });
  ...
  private async getLastCreatedContactId(): Promise<string> {
    ...
    const result = await mcp__surrealdb_emittiv__query({
      query_string:
        'SELECT id FROM contacts WHERE first_name CONTAINS "DELETE ME" ORDER BY created_at DESC LIMIT 1'
    });
