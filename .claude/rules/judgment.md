...
- Local git branch `main` in this repo tracks a stale `github` remote, not `origin` (Forgejo, the real source of truth per CLAUDE.md). Checking local `main` for "is PR #N merged yet" gives a wrong (stale) answer - this handover itself was found stale that way on 2026-07-02. Always check `origin/main` (`git log origin/main` or the Forgejo API/MCP) before asserting merge state.
