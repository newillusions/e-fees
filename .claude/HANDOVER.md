...
## Current Status
**Very large session 2026-06-14.** Six workstreams landed; all committed + pushed to Forgejo `main`. Working tree clean except this handover.

- **Versions** (unchanged): desktop 0.16.0, e-fees-api 0.3.4, e-fees-scope 0.2.0.
- **Branch**: main. Today's commits: `22731a4` (partial-update fix), `80c2ee9` (smoke revert), `70f2983` (agent slim-rewrite), `38fff80` (clause regression test), `40615be` (assumptions + IDW T5 docs), `7e7d228` (handover), `187a7a2` (FP clause corpus), `4025353` (clause-library audit + assumptions conclusion).
...
5. **FP clause corpus mined** (`obs:95ouof2hq1o5twn1il0a`). All **69 historical fee proposals** (50 projects, the `*-FP-*.pdf` under `02 Proposal/` across 00 Inactive/11 Current/99 Completed on `ssh unraid` Nextcloud) → pdftotext → 93-agent Workflow → **1,683 clauses / 23 types**. Deliverables in `docs/clause-corpus/`: `archive/` (69 verbatim per-doc), `CLAUSE-CATALOG.md` (deduped variants + canonical), `INDEX.md`. Supersedes the partial 51-doc `proposal_corpus` ingest.
6. **Assumptions resolved + clause-library audit**. Assumptions = standard preamble + one reusable coordination-boundary block + site-visit line; rest project-specific. Added standard **Assumptions** clause to the live library (`clause:paw4ejkcmhtwpm0l2miq`, Legal, default — library had none before). `docs/clause-corpus/LIBRARY-AUDIT.md` audits all 21 curated clauses vs the catalog: 9 OK, 4 thin, 4 divergent/outdated, 7 type-gaps.
...
2. **Assumptions clause** — RESOLVED via corpus mining (`docs/plans/assumptions-review.md` rewritten). Standard Assumptions clause added to library. Open: whether to make it toggleable in scope-assembly.
3. **Clause-library backlog** (from `docs/clause-corpus/LIBRARY-AUDIT.md`, awaiting Martin) — fix 4 divergent clauses (Defined Role missing regs paragraph; Fees/Payment Terms missing variation notice + 30→14-day; Proposal Validity → "sixty [60] days"; Prepared By → real signature block); supplement 4 thin; add 7 gap clauses (biggest: **Scope & Services** appointment+areas block, most-used family, absent). PUT/POST /clauses against prod 10.0.23.11 ns emittiv.
4. **Extended colour palette** — RESOLVED/stale: accent tokens already live in `app.css`. Martin to confirm the shipped mappings are intended or flag any to revisit. (`memory/project_color_system.md` updated.)
5. **IDW T5 `.indd` linking** — scoped in `docs/plans/2026-06-14-idw-t5-indd-linking-scope.md`. One-time InDesign **Place** of the (already-generated) T5 Reimbursable Costs sheet into story 25 after T4. Execute manually or via UXP MCP with InDesign open.
6. **Lulu 26-97104** — waiting on client meeting (price not locked); then model Acoustics 55k as a discipline + regenerate docs.
