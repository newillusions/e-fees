...
/// ```
#[tauri::command]
pub async fn get_fees(state: State<'_, AppState>) -> Result<Vec<Fee>, String> {
    info!("Fetching fees from database");
    ...
    match manager_clone.get_fees().await {
        Ok(fees) => {
            info!("Successfully fetched {} fees", fees.len());
            Ok(fees)
        }
        Err(e) => {
...
/// ```
#[tauri::command]
pub async fn create_fee(fee: FeeCreate, state: State<'_, AppState>) -> Result<Fee, String> {
    info!("Creating new fee: {}", fee.name);
    info!("Fee project_id: {}", fee.project_id);
    info!("Fee company_id: {}", fee.company_id);
    info!("Fee contact_id: {}", fee.contact_id);
    ...
    match manager_clone.create_fee(fee).await {
        Ok(created_fee) => {
            info!("Successfully created fee with id: {:?}", created_fee.id);
            Ok(created_fee)
        }
        Err(e) => {
            error!("Failed to create fee: {}", e);
...
pub async fn update_fee(id: String, fee: FeeUpdate, state: State<'_, AppState>) -> Result<Fee, String> {
    info!("=== Modal Update Fee Called ===");
    info!("ID received from modal: '{}'", id);
    info!("Fee data received: name='{}', number='{}', status='{:?}'", fee.name, fee.number, fee.status);
    ...
    match manager_clone.update_fee(&id, fee).await {
        Ok(updated_fee) => {
            info!("Successfully updated fee with id: {}", id);
            Ok(updated_fee)
        }
        Err(e) => {
            error!("Failed to update fee with id '{}': {}", id, e);
...
/// ```
#[tauri::command]
pub async fn delete_fee(id: String, state: State<'_, AppState>) -> Result<Fee, String> {
    info!("Deleting fee with id: {}", id);
    ...
    match manager_clone.delete_fee(&id).await {
        Ok(deleted_fee) => {
            info!("Successfully deleted fee with id: {}", id);
            Ok(deleted_fee)
        }
        Err(e) => {
            error!("Failed to delete fee: {}", e);
