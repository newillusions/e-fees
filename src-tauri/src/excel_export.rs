...
use crate::db::types::{
    Discipline, Fee, PostContractItem, PricingBreakdown, PricingCell, ReimbursableCost, Stage,
};
...
fn fmt_value() -> Format {
    Format::new().set_font_name("Montserrat").set_font_size(10)
}
...
// ============================================================================
// TEMPLATE POPULATION (-PRI) — now shared in e-fees-core
// ============================================================================

/// The internal `-PRI` pricing-template generator lives in `e-fees-core`
/// (shared with the standalone API). Re-exported so the desktop command layer
/// keeps importing it from `crate::excel_export`.
pub use e_fees_core::export::fee_template::generate_fee_template;
...
pub fn generate_fee_excel(fee: &Fee, output_path: &Path) -> Result<String, String> {
    ...
    // ------------------------------------------------------------------
    worksheet
        .merge_range(row, 0, row, 4, &fee.name, &fmt_title())
        .map_err(|e| e.to_string())?;
    row += 1;
    ...
    for (label, value) in &header_fields {
        worksheet
            .write_string_with_format(row, 0, *label, &fmt_label())
            .map_err(|e| e.to_string())?;
        worksheet
            .write_string_with_format(row, 1, value, &fmt_value())
            .map_err(|e| e.to_string())?;
        row += 1;
    }
...
fn write_pricing_section(
    ...
    // Section title
    ws.merge_range(row, 0, row, 4, "Fee Breakdown", &fmt_section_header())
        .map_err(|e| e.to_string())?;
    row += 1;
    ...
    for (label, value) in &config_fields {
        ws.write_string_with_format(row, 0, *label, &fmt_label())
            .map_err(|e| e.to_string())?;
        ws.write_string_with_format(row, 1, value, &fmt_value())
            .map_err(|e| e.to_string())?;
        row += 1;
    }
    row += 1;
    ...
    if !pricing.disciplines.is_empty() && !pricing.stages.is_empty() {
        row = write_discipline_stage_matrix(
            ws,
            row,
            &pricing.disciplines,
            &pricing.stages,
...
fn write_discipline_stage_matrix(
    ...
    ws.write_string_with_format(row, 0, "Discipline", &fmt_col_header())
        .map_err(|e| e.to_string())?;
    for (i, stage) in design_stages.iter().enumerate() {
        ws.write_string_with_format(row, (i + 1) as u16, &stage.code, &fmt_col_header())
            .map_err(|e| e.to_string())?;
    }
    ws.write_string_with_format(
        row,
        (design_stages.len() + 1) as u16,
        "Total",
        &fmt_col_header(),
    )
    .map_err(|e| e.to_string())?;
    ...
    for disc in disciplines {
        ws.write_string_with_format(row, 0, &disc.name, &fmt_cell_header())
            .map_err(|e| e.to_string())?;
        ...
        for (j, stage) in design_stages.iter().enumerate() {
            let amount = cells
                ...
                .unwrap_or(0.0);
            disc_total += amount;
            ws.write_number_with_format(row, (j + 1) as u16, amount, &fmt_cell_data())
                .map_err(|e| e.to_string())?;
        }
    ...
    // Stage totals row
    ws.write_string_with_format(row, 0, "Stage Total", &fmt_total_label())
        .map_err(|e| e.to_string())?;
    for (j, stage) in design_stages.iter().enumerate() {
        let stage_total: f64 = cells
            .iter()
            .filter(|c| c.stage_id == stage.id)
            .map(|c| c.override_amount.unwrap_or(c.amount))
            .sum();
        ws.write_number_with_format(row, (j + 1) as u16, stage_total, &fmt_total_currency())
            .map_err(|e| e.to_string())?;
    }
    // Grand total of all design cells
    let all_design_total: f64 = cells
        .iter()
...
fn write_post_contract_section(
    ws: &mut Worksheet,
    start_row: u32,
    ...
    ws.merge_range(row, 0, row, 4, "Post-Contract Items", &fmt_section_header())
        .map_err(|e| e.to_string())?;
    row += 1;
    ...
    for (i, h) in headers.iter().enumerate() {
        ws.write_string_with_format(row, i as u16, *h, &fmt_col_header())
            .map_err(|e| e.to_string())?;
    }
    row += 1;
    ...
    for item in items {
        ws.write_string_with_format(row, 0, &item.description, &fmt_value())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 1, item.quantity, &fmt_value())
            .map_err(|e| e.to_string())?;
        ws.write_string_with_format(row, 2, &item.unit, &fmt_value())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 3, item.rate, &fmt_currency())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 4, item.amount, &fmt_currency())
            .map_err(|e| e.to_string())?;
        total += item.amount;
        row += 1;
    }

    ws.write_string_with_format(row, 0, "Post-Contract Total", &fmt_total_label())
        .map_err(|e| e.to_string())?;
    ws.write_number_with_format(row, 4, total, &fmt_total_currency())
        .map_err(|e| e.to_string())?;
    row += 2;
...
fn write_reimbursable_costs_section(
    ws: &mut Worksheet,
    start_row: u32,
    ...
    ws.merge_range(row, 0, row, 4, "Reimbursable Costs", &fmt_section_header())
        .map_err(|e| e.to_string())?;
    row += 1;
    ...
    let mut total = 0.0_f64;
    for cost in costs {
        ws.write_string_with_format(row, 0, &cost.description, &fmt_value())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 1, cost.base_cost, &fmt_currency())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 2, cost.markup_percent / 100.0, &fmt_percent())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 3, cost.cost_to_client, &fmt_currency())
            .map_err(|e| e.to_string())?;
        ws.write_string_with_format(row, 4, cost.notes.as_deref().unwrap_or(""), &fmt_value())
            .map_err(|e| e.to_string())?;
        total += cost.cost_to_client;
        row += 1;
    }

    ws.write_string_with_format(row, 0, "Costs Total", &fmt_total_label())
        .map_err(|e| e.to_string())?;
    ws.write_number_with_format(row, 3, total, &fmt_total_currency())
        .map_err(|e| e.to_string())?;
    row += 2;
...
fn write_summary_section(
    ...
    ws.merge_range(row, 0, row, 4, "Summary", &fmt_section_header())
        .map_err(|e| e.to_string())?;
    row += 1;
    ...
    for (label, amount) in &summary_lines {
        ws.write_string_with_format(row, 0, *label, &fmt_label())
            .map_err(|e| e.to_string())?;
        ws.write_number_with_format(row, 1, *amount, &fmt_currency())
            .map_err(|e| e.to_string())?;
        row += 1;
    }

    // Grand total — prominent
    ws.write_string_with_format(row, 0, "GRAND TOTAL", &fmt_total_label())
        .map_err(|e| e.to_string())?;
    ws.write_number_with_format(row, 1, pricing.grand_total, &fmt_total_currency())
        .map_err(|e| e.to_string())?;
    row += 1;
    ...
    ws.write_string_with_format(
        row,
        0,
        &format!("All amounts in {}", pricing.config.currency),
        &fmt_label(),
    )
    .map_err(|e| e.to_string())?;
    row += 1;
...
mod tests {
    ...
    fn full_fee() -> Fee {
        ...
        fee.pricing = Some(crate::db::types::json_to_dbvalue(
            &serde_json::to_value(PricingBreakdown {
                ...
                stages: vec![
                    ...
                    Stage {
                        id: "cd".to_string(),
                        name: "Construction Documents".to_string(),
                        code: "CD".to_string(),
                        percentage: 50.0,
                        order: 3,
                        is_post_contract: false,
                ...
                cells: vec![
                    ...
                    PricingCell {
                        discipline_id: "lx".to_string(),
                        stage_id: "dd".to_string(),
                        amount: 22050.0,
                        override_amount: None,
                    },
                    PricingCell {
                        discipline_id: "lx".to_string(),
                        stage_id: "cd".to_string(),
                        amount: 36750.0,
                        override_amount: None,
                    },
                    PricingCell {
                        discipline_id: "ctrl".to_string(),
                        stage_id: "sd".to_string(),
                        amount: 6300.0,
                        override_amount: None,
                    },
    ...
    fn test_full_fee_generates_valid_xlsx() {
        ...
        assert!(
            metadata.len() > 1000,
            "Generated file seems too small: {} bytes",
            metadata.len()
        );
    ...
    fn test_xlsx_file_has_valid_magic_bytes() {
        ...
        assert_eq!(
            &bytes[0..2],
            b"PK",
            "File does not have ZIP/XLSX magic bytes"
        );
    ...
    fn test_template_with_5_stages_fallback() {
        ...
        if let Some(mut pricing) = fee.pricing_typed() {
            ...
            pricing.stages.push(Stage {
                id: "ca".to_string(),
                name: "Construction Admin".to_string(),
                code: "CA".to_string(),
                percentage: 10.0,
                order: 5,
                is_post_contract: false,
        ...
        assert!(
            result.is_ok(),
            "generate_fee_template (5 stages) failed: {:?}",
            result.err()
        );
    ...
    fn test_template_fallback_embedded() {
        ...
        assert!(
            result.is_ok(),
            "Embedded template fallback failed: {:?}",
            result.err()
        );
